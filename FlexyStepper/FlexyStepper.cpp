...
//
FlexyStepper::FlexyStepper(PinName stepPinNumber, PinName directionPinNumber) : stepPin(stepPinNumber), directionPin(directionPinNumber)
{
...
bool FlexyStepper::moveToHomeInSteps(long directionTowardHome,  
  ...
  PinName homeLimitSwitchPinName)
{
  float originalDesiredSpeed_InStepsPerSecond;
  bool limitSwitchFlag;
  
  DigitalIn homeLimitSwitchPin(homeLimitSwitchPinName);
...
bool FlexyStepper::moveToHomeInStepsInvert(long directionTowardHome,  
  float speedInStepsPerSecond, long maxDistanceToMoveInSteps, 
  PinName homeLimitSwitchPinName)
{
  float originalDesiredSpeed_InStepsPerSecond;
...
{ 
  ...
  if (directionOfMotion == 0)
  {
    ...
    {
      directionOfMotion = 1;
      directionPin = POSITIVE_DIRECTION;
      nextStepPeriod_InUS = periodOfSlowestStep_InUS;
      lastStepTime_InUS = us_ticker_read(); 
      return(false);
    }
    ...
    {
      directionOfMotion = -1;
      directionPin = NEGATIVE_DIRECTION;
      nextStepPeriod_InUS = periodOfSlowestStep_InUS;
      lastStepTime_InUS = us_ticker_read(); 
      return(false);
    }
  ...
  //
  currentTime_InUS = us_ticker_read();
